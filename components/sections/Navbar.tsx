        ...
        <>
            <nav className="w-screen items-center flex justify-between overflow-hidden px-5 md:px-20 h-20 bg-seconday-2">
                <div className="flex items-center justify-between w-full md:w-auto">
                    ...
                    {isOpen && <div className="bg-secondary fixed flex flex-col justify-center items-center overflow-hidden top-0 left-0 z-50">
                        {navItems.map((item: nav, index: number) => {
                        return (
                            <Link key={index} className="group transition-all" href={item.link}>
                                {item.title}
                ...
                <div className={`hidden md:flex-row md:flex font-sans text-lg text-secondary gap-10 ${isOpen ? 'flex' : 'hidden'} md:flex`}>
                    {navItems.map((item: nav, index: number) => {
                        return (
                            <Link key={index} className="group transition-all" href={item.link}>
                                {item.title}
